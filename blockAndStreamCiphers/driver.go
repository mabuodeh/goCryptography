//
// fmt.Println("Challenge 7")
// cha7ByteData := set1.OpenTextFile(fileDirectory + "7b.txt")
// fmt.Println(set1.EncryptEcb(cha7ByteData, []byte("YELLOW SUBMARINE")))
// cha7ByteData = set1.OpenBase64File(fileDirectory + "7.txt")
// fmt.Println(string(set1.DecryptEcb(cha7ByteData, []byte("YELLOW SUBMARINE"))))
//
// fmt.Println("Challenge 8")
// cha8ByteData := set1.OpenHexFile(fileDirectory + "8.txt")
// // fmt.Println(cha8ByteData)
// fmt.Println(set1.DetectAesEcbLine(cha8ByteData))
package blockAndStreamCiphers
